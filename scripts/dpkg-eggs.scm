...
(define opts
  `(
    ,(args:make-option (eggdir)       (required: "DIR")    
...
(define (build-deb eggdir output-dir path)
  (let* ((name     (pathname-strip-directory path))
	...
	(let ((start (cwd)))
	  ...
	  (run (rm -rf ,(s+ output-dir dirsep name)))
	  (run (cp -R ,release ,(s+ output-dir dirsep name)))
	  (run (cp -R ,debdir ,(s+ output-dir dirsep name)))
	  (cd  (s+ output-dir dirsep name))
...
(define (main options operands)
  (let ((opt_eggdir  (alist-ref 'eggdir options))
    ...
    (let ((eggdirs (filter-map 
      ...
      (for-each (lambda (x) (build-deb opt_eggdir opt_output-dir x))
		eggdirs))))
